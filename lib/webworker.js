...
//   addToWorkerContext: A callback allowing injection of globals (e.g., for polyfilling)
//}
module.exports = function (workerConfig) {
    ...
    if (workerConfig.permittedProtocols && !Array.isArray(workerConfig.permittedProtocols)) {
        throw new TypeError('The permittedProtocols property must be an array');
    }
    if (workerConfig.addToWorkerContext && typeof workerConfig.addToWorkerContext !== 'function') {
        throw new TypeError('The addToWorkerContext property must be a function');
    }
    // A Web Worker
    //
    ...
    var Worker = function(src, opts) {
        ...
        var start = function() {
            ...
            httpServer.addListener('listening', function() {
                ...
                postMessageImpl(wwutil.MSGTYPE_EVAL, {evaluate: function (workerCtxGlobal, workerCtx) {
                    console.log('worked');
                    workerConfig.addToWorkerContext(workerCtxGlobal, workerCtx);
                }});
