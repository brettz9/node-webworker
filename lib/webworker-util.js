...
var MsgStream = function(s) {
    ...
    self.send = function(v, fd) {
        ...
        if (v && v[0] === exports.MSGTYPE_EVAL) {
            s.send(bson.serialize(ms, {serializeFunctions: true}), {binary: true, mask: true});
            return;
        }
    ...
    s.on('message', function(ms) {
        ...
        if (Array.isArray(msg) && msg[0] === exports.MSGTYPE_EVAL) {
            mo = bson.deserialize(ms, {evalFunctions: true});
            fd_seq = mo[0];
            msg = mo[1];
        }
